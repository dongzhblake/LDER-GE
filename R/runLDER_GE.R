...
runLDER_GE <- function(assoc, n.gwas, path, LD.insample=T,  n.ld,method='lder', cores=10, a=NULL, type='jack',size_num=200){
  ...
  if(method=='lder'){
    res <- lder(stats=stats,n.gwas=n.gwas,a=NULL,rough=rough,cores=cores,twostage=!LD.insample,type=type)
    return(res)
  }else if(method=='ldsc'){
    res <- ldsc(stats=stats,n.gwas=n.gwas,a=NULL,cores=cores,twostage=F,type=type)
    return(res)
  }else if(method=='both'){
    res1 <- lder(stats=stats,n.gwas=n.gwas,a=NULL,rough=rough,cores=cores,twostage=!LD.insample,type=type)
