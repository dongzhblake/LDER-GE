...
#' @param n.gwis The sample size of the GWIS summary statistics
#' @param path The path of LD panel directory
#' @param LD.insample T/F, whether the LD reference is estimated with the GWIS cohort (T) or external reference panel (e.g. 1000 Genome Project) (F)
...
#'
#'
runLDER_GE <- function(assoc, n.gwis, path, LD.insample=T,  n.ld,method='lder', cores=10, type='jack',size_num=200){
  library(parallel)
  ...
  if(method=='lder'){
    res <- lder(stats=stats,n.gwas=n.gwas,a=NULL,rough=!LD.insample,cores=cores,twostage=rough,type=type)
    return(res)
  }else if(method=='ldsc'){
    res <- ldsc(stats=stats,n.gwas=n.gwas,a=NULL,cores=cores,twostage=F,type=type)
    return(res)
  }else if(method=='both'){
    res1 <- lder(stats=stats,n.gwas=n.gwas,a=NULL,rough=!LD.insample,cores=cores,twostage=rough,type=type)
